...
for i=1:m
    J=J+sum(Labels(:,i).*log(h(i,:)')+(ones(K,1)-Labels(:,i)).*log(ones(K,1)-h(i,:)'),1)/(-1*m);
end
J=J+(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)))*lambda/(2*m);
D3=a3'-Labels;
D2=(Theta2(:,2:end)'*D3).*sigmoidGradient(z2);

Theta1_grad=Theta1_grad+D2*a1/m;%25*401
Theta2_grad=Theta2_grad+D3*a2/m;%10*26
